...
def test_restore_sets_the_exact_persisted_model_version_not_an_increment(
    ...
    fresh = AnomalyDetector(sample_rate=SAMPLE_RATE)
    assert not fresh.is_fitted

    fresh.restore(fitted_detector.baseline, model_version=7)

    assert fresh.is_fitted
    assert fresh.model_version == 7
    rng = np.random.default_rng(999)
    verdict = fresh.score(_healthy_signal(rng))
    assert verdict.model_version == 7


def test_restore_rejects_a_negative_model_version() -> None:
    detector = AnomalyDetector(sample_rate=SAMPLE_RATE)
    rng = np.random.default_rng(42)
    healthy_windows = [_healthy_signal(rng) for _ in range(10)]
    source = AnomalyDetector(sample_rate=SAMPLE_RATE)
    source.fit(healthy_windows)

    with pytest.raises(ValueError):
        detector.restore(source.baseline, model_version=-1)
