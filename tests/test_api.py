...
from hydra_umc_anomaly_detector.baseline import load_baseline
...
def test_fit_persists_the_baseline_when_a_path_is_configured(tmp_path) -> None:
    ...
    detector = AnomalyDetector(sample_rate=SAMPLE_RATE, threshold=4.0)
    server = DetectorServer(("127.0.0.1", 0), detector, baseline_path)
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    port = server.server_address[1]
    try:
        ...
        windows = [_sine(50.0) for _ in range(5)]
        status, _ = _post(f"{url}/baseline/fit", {"windows": windows})
        assert status == 200

        assert baseline_path.is_file()
        restored, model_version = load_baseline(baseline_path)
        assert model_version == 1
        assert np.array_equal(restored.mean, detector.baseline.mean)
    finally:
        server.shutdown()
        server.server_close()
        thread.join(timeout=2)
